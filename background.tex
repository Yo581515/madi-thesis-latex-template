...
NoSQL databases were introduced to address scalability and inferior schema flexibility witnessed in traditional relational systems. Semi-structured data representations are often used, which can change in form after a given period, as opposed to mandatory relational schema provisions. Document-oriented databases as MongoDB store information in the form of JSON-like documents with the ability to set nested fields in each record or remove some attributes for one record. This is crucial in situations where devices or sources measure different things, and the type of measurement data can change as new measuring devices are added. 
...
Despite these advantages, NoSQL systems usually adopt a relaxed consistency model, as opposed to relational databases.  (\citet{Hellerstein_2007}) indicates that most NoSQL platforms compromise on the transactional guarantees to give precedence to availability and horizontal scalability. Data integrity is a case by which applications are developed to avoid conflicts more especially for many relational constraints or consistency within many records. Temporal queries can also be based on the indexing mechanisms; however, large analytical aggregations may be efficient based on the queries devised and indexing mechanisms.
